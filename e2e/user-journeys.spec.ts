...
test.describe('User Journeys - Browsing and Searching', () => {
  ...
  test('should display homepage with affiliate links', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="affiliate-links-grid"]')
    ).toBeVisible();

    // Check that at least one affiliate link card is displayed
    await expect(
      page.locator('[data-testid="affiliate-link-card"]').first()
    ).toBeVisible();
  ...
  test('should search for affiliate links', async ({ page }) => {
    ...
    await expect(
      linkCards.first().locator('[data-testid="link-title"]')
  ...
  test('should track clicks on affiliate links', async ({ page }) => {
    ...
    const initialClickCount = parseInt(
      initialClickText?.replace(/[^\d]/g, '') || '0'
    );

    // Click the affiliate link
    const linkButton = firstLink.locator(
    ...
    const updatedClickElement = page
      .locator('[data-testid="affiliate-link-card"]')
      .first()
      .locator('[data-testid="click-count"]');
    const updatedClickText = await updatedClickElement.textContent();
    const updatedClickCount = parseInt(
      updatedClickText?.replace(/[^\d]/g, '') || '0'
    );
  ...
  test('should handle empty search results', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="empty-search-results"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="empty-search-results"]')
    ...
    await expect(
      page.locator('[data-testid="affiliate-link-card"]')
    ).toHaveCount(0);
  });
});

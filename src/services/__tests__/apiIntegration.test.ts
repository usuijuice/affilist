...
describe('API Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const healthResponse = await fetch(`${TEST_API_BASE_URL.replace('/api', '')}/health`);
  ...
  describe('Caching', () => {
    ...
    it('should not cache POST requests', async () => {
      ...
      const response = await apiClient.post('/test-endpoint', { test: 'data' }, { cache: false });
...
export async function isServerAvailable(): Promise<boolean> {
  try {
    const response = await fetch(`${TEST_API_BASE_URL.replace('/api', '')}/health`, {
      method: 'GET',
      timeout: 5000,
    } as RequestInit);
    return response.ok;

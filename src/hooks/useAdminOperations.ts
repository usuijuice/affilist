...
import { getErrorMessage, isAuthError, isNetworkError } from '../utils/apiErrorHandler';
...
export interface UseAdminOperationsReturn {
  ...
  updateLink: (id: string, linkData: Partial<CreateLinkRequest>) => Promise<AffiliateLink | null>;
  deleteLink: (id: string, title: string) => Promise<boolean>;
  bulkDeleteLinks: (ids: string[], titles: string[]) => Promise<boolean>;
  bulkUpdateLinks: (ids: string[], updates: Partial<CreateLinkRequest>) => Promise<boolean>;
...
export function useAdminOperations(): UseAdminOperationsReturn {
  ...
  // Link operations
  const createLink = useCallback(async (linkData: CreateLinkRequest): Promise<AffiliateLink | null> => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  const deleteLink = useCallback(async (id: string, title: string): Promise<boolean> => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  const bulkDeleteLinks = useCallback(async (ids: string[], titles: string[]): Promise<boolean> => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (response.success) {
        notificationService.bulkOperationCompleted('Bulk Delete', response.data.deletedCount);
        return true;
      } else {
  ...
  const bulkUpdateLinks = useCallback(async (
    ids: string[], 
  ...
  // Category operations
  const createCategory = useCallback(async (categoryData: any): Promise<Category | null> => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (response.success) {
        notificationService.success('Category Created', `"${categoryData.name}" has been created successfully.`);
        return response.data;
      } else {
  ...
  const updateCategory = useCallback(async (
    ...
    try {
      ...
      if (response.success) {
        notificationService.success('Category Updated', `"${categoryData.name || 'Category'}" has been updated successfully.`);
        return response.data;
      } else {
  ...
  const deleteCategory = useCallback(async (id: string, name: string): Promise<boolean> => {
    try {
      setIsLoading(true);
      setError(null);
      ...
      if (response.success) {
        notificationService.success('Category Deleted', `"${name}" has been deleted successfully.`);
        return true;
      } else {

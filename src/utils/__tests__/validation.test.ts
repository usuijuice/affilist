...
import { createMockAffiliateLink, createMockCategory, createMockClickEvent, createMockAdminUser, createMockCreateLinkRequest } from '../../test/factories'
...
describe('validateAffiliateLink', () => {
  ...
  it('should return error for description too long', () => {
    const invalidLink = createMockAffiliateLink({ description: 'a'.repeat(1001) })
    const errors = validateAffiliateLink(invalidLink)
  ...
  it('should return error for too many tags', () => {
    const invalidLink = createMockAffiliateLink({ tags: Array(11).fill('tag') })
    const errors = validateAffiliateLink(invalidLink)
...
describe('validateCategory', () => {
  ...
  it('should return error for invalid slug', () => {
    ...
    const errors = validateCategory(invalidCategory)
    expect(errors).toContain('Category slug must contain only lowercase letters, numbers, and hyphens')
  })
  ...
  it('should return error for description too long', () => {
    const invalidCategory = createMockCategory({ description: 'a'.repeat(501) })
    const errors = validateCategory(invalidCategory)
    expect(errors).toContain('Category description must be 500 characters or less')
  })

...
export function createMockAffiliateLink(
  ...
  const defaultLink: AffiliateLink = {
    ...
    description:
      'A fantastic tool that helps developers build amazing applications faster and more efficiently.',
 ...
 */
export function createMockCreateLinkRequest(
  overrides: Partial<CreateLinkRequest> = {}
): CreateLinkRequest {
 ...
 */
export function createMockAffiliateLinks(
  count: number,
  overrides: Partial<AffiliateLink> = {}
): AffiliateLink[] {
...
export function createMockCategories(count: number = 5): Category[] {
  ...
  return Array.from(
    { length: Math.min(count, categoryNames.length) },
    (_, index) =>
      createMockCategory({
        name: categoryNames[index],
        slug: categoryNames[index]
          .toLowerCase()
          .replace(/\s+/g, '-')
          .replace(/[^a-z0-9-]/g, ''),

...
export function AdminDashboard({ initialView = 'dashboard' }: AdminDashboardProps) {
  const { user } = useAuth();
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (response.success) {
        ...
        const activeLinks = response.data.filter(link => link.status === 'active').length;
        const totalClicks = response.data.reduce((sum, link) => sum + link.clickCount, 0);
        const featuredLinks = response.data.filter(link => link.featured).length;
  ...
  const handleUpdateLink = async (linkData: CreateLinkRequest) => {
    ...
    try {
      ...
      const response = await affiliateLinksApi.updateLink(editingLink.id, linkData);
      if (response.success) {
        await loadDashboardData(); // Refresh data
        setEditingLink(null);
        setCurrentView('links');
  ...
  const renderDashboardOverview = () => (
    <div className="space-y-6">
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="p-5">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Featured Links</dt>
                  <dd className="text-lg font-medium text-gray-900">{stats.featuredLinks}</dd>
                </dl>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-4 py-5 sm:p-6">
          <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Recent Links</h3>
          {links.length > 0 ? (
            <div className="space-y-3">
              {links
                ...
                .slice(0, 5)
                .map((link) => (
                  <div key={link.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500">{link.category.name} • {link.clickCount} clicks</p>
                    </div>
        ...
        return (
          <div>
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-600">Add a new affiliate link to your collection</p>
            </div>
            <LinkForm
        ...
        return (
          <div>
            <div className="mb-6">
              ...
              <p className="text-sm text-gray-600">Update the affiliate link information</p>
            </div>
            <LinkForm
  ...
  return (
    <AdminLayout
      title={
        ...
        currentView === 'links' ? 'Manage Links' :
        currentView === 'create' ? 'Create New Link' :
        currentView === 'edit' ? 'Edit Link' :
        'Admin Dashboard'
      }
      subtitle={
        currentView === 'dashboard' ? `Welcome back, ${user?.name}` :
        currentView === 'links' ? 'Manage your affiliate links' :
        currentView === 'create' ? 'Add a new affiliate link' :
        currentView === 'edit' ? 'Update affiliate link information' :
        undefined
      }
    >
      {renderNavigation()}

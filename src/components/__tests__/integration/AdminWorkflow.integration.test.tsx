...
describe('Admin Workflow Integration', () => {
  ...
  it('should create a new affiliate link through the complete workflow', async () => {
    ...
    const mockCreate = vi.mocked(
      require('../../services/affiliateLinksApi').createAffiliateLink
    );
    ...
    await user.type(
      ...
      'A new test link description'
    );
    await user.type(screen.getByLabelText(/url/i), 'https://example.com');
    await user.type(
      screen.getByLabelText(/affiliate url/i),
      'https://example.com?ref=affiliate'
    );
  ...
  it('should edit an existing affiliate link', async () => {
    ...
    const mockUpdate = vi.mocked(
      require('../../services/affiliateLinksApi').updateAffiliateLink
    );
    ...
    const manageLinksButton = screen.getByRole('button', {
      name: /manage links/i,
    });
  ...
  it('should delete an affiliate link with confirmation', async () => {
    ...
    const mockDelete = vi.mocked(
      require('../../services/affiliateLinksApi').deleteAffiliateLink
    );
  ...
  it('should handle form validation errors gracefully', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/affiliate url is required/i)
      ).toBeInTheDocument();
    });

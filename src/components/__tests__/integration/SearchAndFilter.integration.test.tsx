...
const mockCategories = [
  createMockCategory({ id: '1', name: 'Web Development', slug: 'web-development' }),
...
describe('Search and Filter Integration', () => {
  ...
  it('should filter links by search query and category', async () => {
    ...
    const webDevCategory = screen.getByRole('button', { name: /web development/i });
  ...
  it('should sort filtered results correctly', async () => {
    ...
    await waitFor(() => {
      const linkTitles = screen.getAllByTestId('link-title').map(el => el.textContent);
  ...
  it('should maintain filter state when switching between sort options', async () => {
    ...
    // Apply category filter
    const designCategory = screen.getByRole('button', { name: /design tools/i });
  ...
  it('should restore filters from URL parameters on page load', async () => {
    ...
    window.history.pushState({}, '', '/?search=Figma&category=design-tools&sort=commission');

...
describe('HighlightedText', () => {
  ...
  it('highlights matching text case-insensitively', () => {
    ...
    expect(container?.innerHTML).toContain(
      '<mark class="bg-yellow-200 px-1 rounded">World</mark>'
    );
  });
  ...
  it('handles special regex characters in search query', () => {
    render(
      <HighlightedText text="Price: $10.99 (special)" searchQuery="$10.99" />
    );

    const container = screen.getByText(/Price/).parentElement;
    expect(container?.innerHTML).toContain(
      '<mark class="bg-yellow-200 px-1 rounded">$10.99</mark>'
    );
  });

  it('handles parentheses in search query', () => {
    render(
      <HighlightedText text="Price: $10.99 (special)" searchQuery="(special)" />
    );

    const container = screen.getByText(/Price/).parentElement;
    expect(container?.innerHTML).toContain(
      '<mark class="bg-yellow-200 px-1 rounded">(special)</mark>'
    );
  });
  ...
  it('handles partial word matches', () => {
    render(
      <HighlightedText text="JavaScript is awesome" searchQuery="Script" />
    );

    const container = screen.getByText(/Java/).parentElement;
    expect(container?.innerHTML).toContain(
      '<mark class="bg-yellow-200 px-1 rounded">Script</mark>'
    );
  });
});

...
import type { AffiliateLinkFilters, PaginationOptions } from '../database/models/types.js';

const router = Router();

// GET /api/links/featured - Get featured links (must come before /links/:id)
router.get('/links/featured', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/links/popular', asyncHandler(async (req: Request, res: Response) => {
  const { limit = '10' } = req.query;
  const parsedLimit = Math.min(Math.max(parseInt(limit as string) || 10, 1), 50);
...
router.get('/links', asyncHandler(async (req: Request, res: Response) => {
  ...
  const validSortFields = ['created_at', 'updated_at', 'title', 'click_count', 'commission_rate'];
...
router.get('/links/:id', asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;

...
router.get('/categories', asyncHandler(async (req: Request, res: Response) => {
  ...
  if (include_counts === 'true') {
    ...
    const categoriesWithCounts = await CategoryModel.getCategoriesWithLinkCount();
    
    // Apply pagination manually since getCategoriesWithLinkCount doesn't support it
    const paginatedCategories = categoriesWithCounts.slice(parsedOffset, parsedOffset + parsedLimit);

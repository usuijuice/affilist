  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const id of ids) {
      try {
        ...
        results.failed++;
        results.errors.push(
          `Failed to delete link ${id}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    for (const id of ids) {
      try {
        ...
        results.failed++;
        results.errors.push(
          `Failed to update link ${id}: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
